...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Dobot und Mazerunner}
...
\caption{Kennung, Komponente und Funktion im gemeinsamen Tischaufbau aus
    ...
    folgt separat (1--20 Maze-Runner, A--E Dobot Magician)}
...
4 & Drehscheibe & Rotation des Rundtakttisches \\
5 & Kugelmagazinstation & Zuführung Kugeln \\
6 & Erste Schwenkarmstation & Deckeltransfer zur Drehscheibe \\
7 & Presse & Fügt Boden und Deckel zusammen \\
8 & Ausgabeschwenkarmstation & Produktausgabe \\
9 & Steuer-Buttons & Manuelle Stationsbedienung \\
10 & Not-Aus-Knopf & Sicherheitsabschaltung \\
11 & Treiberplatinen & Schrittmotoransteuerung \\
12 & 230-V-Zufuhr & Netzanschluss \\
13 & 24-V-Netzteil & Ein-/Ausgangsversorgung \\
14 & Verteilerschienen & Elektrische Verteilung \\
15 & SPS~1 & Hauptsteuerung \\
16 & SPS~2 & Steuerung Ausgabeschwenkarmstation \\
17 & VPN-Router & Fernzugriff \\
18 & Magnetventile & Druckluftsteuerung \\
19 & Druckminderer & Reduzierung auf vier~Bar \\
20 & Druckluftzufuhr & Versorgung durch Laborkompressor \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{TIA_Portal_V17}
...
Im Folgenden wird gezeigt, wie auf Grundlage der Inhalte der vorangegangenen drei Abschnitte eine NVIDIA-Isaac-Sim-Extension aufgebaut wird.
...
Diese aus zwölf Modulen bestehende Extension wird im Folgenden als \texttt{Mazerunner\_DT\_IsaacSim\_Extension}, kurz Mazerunner-Extension, bezeichnet.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{MazeRunnerIBD}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Ist-Zustand_Mazerunner}
...
Für den Betrieb mit dem Arbeitsdruck von vier Bar musste ein Druckluftminderer, wie in Abbildung~\ref{fig:dobot_mazerunner_tisch} mit Nummer 19 gekennzeichnet, mit den passenden Anschlüssen verbaut werden, der eingangsseitig per Schnellkupplung mit dem Kompressor verbunden ist und ausgangsseitig über den sechs Millimeter durchmessenden Schlauch die in Abbildung~\ref{fig:dobot_mazerunner_tisch} mit Nummer 18 gekennzeichneten Magnetventile speist.

Die netzwerkseitige Inbetriebnahme begann mit der Konfiguration des VPN-Routers der Maze-Runner-Anlage, wie in Abbildung~\ref{fig:dobot_mazerunner_tisch} mit Nummer 17 gekennzeichnet ist, der zunächst in das Netzwerk der Digitalen Fabrik im Labor aufgenommen und freigegeben werden musste, damit er die HSRM und darüber den \emph{DigitalTwinService} erreichen kann.
Anschließend musste sichergestellt werden, dass die SPS im selben Subnetz wie der Router konfiguriert ist, sodass beide SPS-Anlagen (Nummern 15 und 16 in Abbildung~\ref{fig:dobot_mazerunner_tisch}) über den Router im selben Netz erreichbar sind.
...
Die aus der Schuko-Steckdose gespeisten 230\,V werden dabei über diesen Schalter auf die Eingänge des neuen WAGO-Netzteils geschaltet, das mit Nummer 13 in Abbildung~\ref{fig:dobot_mazerunner_tisch} gekennzeichnet ist und die gesamte Anlage anschließend mit 24\,V Gleichspannung versorgt.
Dabei wurde außerdem der zunächst nicht angeschlossene Not-Aus-Schalter ergänzt, mit Nummer 10 in Abbildung~\ref{fig:dobot_mazerunner_tisch} gekennzeichnet, welcher die Versorgungsspannung der Aktorik separat von der SPS-Versorgungsspannung spannungsfrei schaltet, wodurch die SPS im Not-Aus-Fall nicht heruntergefahren wird.
...
Die treppenförmige Halterung der Schrittmotortreiber, mit Nummer 11 in Abbildung~\ref{fig:dobot_mazerunner_tisch} gekennzeichnet, wurde von der HSRM entwickelt, wie in Abbildung~\ref{fig:schrittmotortreiber} im Detail dargestellt ist.
...
So ist bei der im Vordergrund sichtbaren Platine der Drehscheibe (Nummer 4 in Abbildung~\ref{fig:dobot_mazerunner_tisch}) nur der Pin \texttt{DIR-} angeschlossen, während bei der dahinterliegenden Platine des Schwenkarms beide Direction-Pins für die Vorwärts- und Rückwärtsbewegung angeschlossen sind.
...
Dieser besteht aus einem Saugkopf mit Luftschlauchanschluss sowie einer separaten Pumpeneinheit, die über die digitalen Anschlüsse GP1 (General Purpose) und Switch~1 (SW1) angesteuert wird, wie Abbildung~\ref{fig:dobot_sauggreifer} zeigt~\cite{kuester2026dobottutorial}.
Die Pumpe wird mit der Steuerung des Dobot Magician verbunden, während der Servomotor (J4), der oben am Unterarm über Port GP3 angeschlossen ist, den unteren Teil des Endeffektors um die Z-Achse dreht, wie Abbildung~\ref{fig:dobot_sauggreifer_anschluss} zeigt~\cite{kuester2026dobottutorial}.
...
Das Konzept sieht eine eigenständige Kit-Extension namens \texttt{Dobot\_DT\_IsaacSim\_Extension}, im Folgenden kurz Dobot-Extension genannt, vor, die über die \emph{pydobot}-Bibliothek~\cite{pydobot_docs} zyklisch die Pose des realen Dobot Magician ausliest und daraus die Gelenkwinkel J1--J4 berechnet, welche an die USD-Stage weitergeleitet werden.
...
Abbildung~\ref{fig:dobot_urdf_aufbau} zeigt diesen Ablauf, der im Folgenden kurz erläutert wird und sich am Workflow eines von NVIDIA bereitgestellten Tutorials zum Import und zur Assemblierung von Manipulatoren in NVIDIA Isaac Sim orientiert~\cite{nvidia_isaacsim_manipulator_tutorial}.
...
\begin{figure}[H]
    \hspace*{-2cm}\includegraphics[width=19cm]{dobot_wege_v20_cropped}
...
Der am Sauggreifer montierte Servomotor wird an GP3 angeschlossen, in diesem Anwendungsfall jedoch nicht genutzt, da er sich nicht über reguläre Bewegungsbefehle, sondern nur über die Low-Level-Kommandos \texttt{SetIOMultiplexing} und \texttt{SetIOPWM} ansteuern lässt, für die \emph{pydobot} lediglich eine rohe \texttt{set\_io()}-Methode ohne Winkelabstraktion bereitstellt~\cite{dobot_communication_protocol,pydobot_docs}.
