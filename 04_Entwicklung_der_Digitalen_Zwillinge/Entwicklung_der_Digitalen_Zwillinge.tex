...
Das proprietäre, seriell-basierte USB-Protokoll des Dobot Magician ist nicht nativ in Isaac Sim integriert, was eine Middleware-Schicht notwendig macht.
...
Das Konzept sieht eine eigenständige Kit-Extension namens \texttt{Dobot\_DT\_IsaacSim\_Extension} vor, deren Klasse \texttt{DobotROSNode} über die \emph{pydobot}-Bibliothek~\cite{pydobot_docs} zyklisch die Pose des realen Roboters ausliest und daraus die Gelenkwinkel J1--J4 berechnet.
...
Dieses bildet eine geschlossene kinematische Kette, wobei zwei mechanische Pfade zwischen Schultergelenk und Werkzeugplatte gegenseitige Abhängigkeiten erzeugen, die über einfache Vorwärtskinematik nicht auflösbar sind.
...
Anstelle der physischen Gelenke J3 und J4 definiert das Modell die aktiven Gelenke \texttt{joint\_5} und \texttt{joint\_6}, wobei \texttt{joint\_5} als Differenz der realen Schulter- und Ellbogenwinkel zurückgerechnet wird und dadurch den Unterarm in Weltkoordinaten horizontal hält, während \texttt{joint\_6} den Endeffektor unabhängig von der Armkonfiguration senkrecht ausrichtet.
...
Die Datenkopplung zwischen realem Roboter und NVIDIA Isaac Sim wird vollständig über die USB-Verbindung realisiert, über die der Dobot Magician mittels der \emph{pydobot}-Bibliothek an die Extension angebunden ist.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{dobot_wege_1_2_v11}
    \caption{Datenflussstruktur der Dobot-Magician-Extension zwischen realem Roboter und NVIDIA Isaac Sim.}
    \label{fig:dobot_wege}
\end{figure}

Im oberen Pfad werden die aktuellen Gelenkwerte des Dobot Magician ausgelesen und anschließend in die USD-Szene übertragen.
...
\texttt{\_sim\_updater()} ruft die Methode \texttt{real\_read\_joints()} der Klasse \texttt{DobotROSNode} auf, welche über \emph{pydobot} den Befehl \texttt{device.pose()} absetzt.
...
Der Roboter antwortet mit der aktuellen XYZ-Koordinate sowie der Werkzeugkopfdrehung um die Z-Achse in Millimetern und Grad, woraus \emph{pydobot} ein Python-Tupel mit den Gelenkstellungen J1 bis J4 bildet.
...
Auf Weg~2 löst ein Klick im Dobot-HMI eine der \texttt{real\_*}-Methoden aus, etwa für Jog-Befehle, Vakuumgreifer-Schaltungen oder das Anfahren der Home-Position.
...
Da beide Wege parallel und unabhängig voneinander laufen, liest \texttt{real\_read\_joints()} im nächsten Takt stets die aktuelle Ist-Position aus, unabhängig davon, ob über Weg~2 noch eine Bewegung läuft.
Damit ist eine funktionale Synchronisation im Sinne eines Digitalen Schattens nach Kritzinger et al.\ realisiert, die um die auf Weg~2 beschriebene, manuell auslösbare Fernsteuerung in Gegenrichtung ergänzt wird, während eine durchgängig automatisierte bidirektionale Kopplung Gegenstand zukünftiger Arbeiten bleibt~\cite{kritzinger2018}.

\subsubsection{Inbetriebnahme}
\label{subsubsec:dobot_inbetriebnahme}

Für die Inbetriebnahme wird der Dobot Magician zunächst in eine im FDM-Druckverfahren gefertigte Halterung eingesetzt, die auf dem Tisch der Maze-Runner-Anlage verschraubt ist.
Anschließend wird der Dobot Magician über das mitgelieferte Netzteil mit Spannung versorgt und per USB an den Laptop angeschlossen.
Der in Abbildung~\ref{fig:dobot_sauggreifer} und Abbildung~\ref{fig:dobot_sauggreifer_anschluss} gezeigte Sauggreifer wird in die Werkzeugführung eingeführt, mit der mitgelieferten Schraube fixiert und anschließend über die Schnittstellen GP1 und GP3 sowie die Versorgungsstecker für Vakuumpumpe und Servomotor verkabelt, wobei die Vakuumpumpe das notwendige Vakuum erzeugt.

Das Modellauto des DeLorean und der das FTS abstrahierende Würfel, auf dem die Schutzkappe abliegt, werden auf demselben Tisch an mit Bleistift markierten Stellen platziert, deren Abstände in die Isaac-Sim-Szene übernommen und durch die Simulation verifiziert werden.
Die FDM-gefertigte Halterung ist auch in Abbildung~\ref{fig:dobot_pp_szenario} und Abbildung~\ref{fig:dobot_labor} zu sehen.
